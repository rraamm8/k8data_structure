...
class LinkedList1 {
	...
	public void Add(int element) {// 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
		...
		System.out.println("추가할 값: " + element);
		
		if (first == null || first.data >= element) {// insert into empty list
			newNode.link = first;
			first = newNode;
			System.out.println("리스트 첫 부분에 추가: " + element);
			return;
		}
		Node1 current = first;
		...
		System.out.println("리스트 중간에 추가: " + element);
	}
